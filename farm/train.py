...
from torch.nn import DataParallel, Linear, Module, ModuleList, MSELoss
from torch.optim import Optimizer
from torch.optim.lr_scheduler import _LRScheduler
from tqdm import tqdm

from farm.data_handler.data_silo import DataSilo, DistillationDataSilo
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.from_epoch, self.epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                if self.tracker:
                    self.tracker.log_metrics({f"TrainLoss": float(loss.detach().cpu().numpy())}, step=self.global_step)
                if self.log_learning_rate:
                    self.tracker.log_metrics({"LearningRate": self.lr_schedule.get_last_lr()[0]}, step=self.global_step)
                ...
                if (
                    ...
                    if dev_data_loader is not None:
                        ...
                        evaluator_dev.log_results(result, "D.E.V.", self.global_step)
        ...
        if self.evaluator_test and self.local_rank in [0, -1]:
            ...
            if test_data_loader is not None:
                evaluator_test = Evaluator(
                    data_loader=test_data_loader, tasks=self.data_silo.processor.tasks, device=self.device, logger=self.tracker
                )
                self.test_result = evaluator_test.eval(self.model)
                evaluator_test.log_results(self.test_result, "T.E.S.T", self.global_step)
        return self.model
    ...
    def backward_propagate(self, loss: torch.Tensor, step: int):
        ...
        if self.global_step % self.log_loss_every == 0 and self.local_rank in [-1, 0]:
            if self.local_rank in [-1, 0] and self.tracker is not None:
    ...
    def log_params(self):
        params = {"epochs": self.epochs, "n_gpu": self.n_gpu, "device": self.device, "step": self.global_step}
        if self.tracker:
...
class DistillationLoss(Module):
    ...
    def forward(self, batch):
        ...
        for student_attention, teacher_attention, dim_mapping in zip(
            ...
            student_attention = torch.where(student_attention <= -1e2, torch.zeros_like(student_attention), student_attention)
            teacher_attention = torch.where(teacher_attention <= -1e2, torch.zeros_like(teacher_attention), teacher_attention)
