...
class Evaluator:
    ...
    def __init__(self, data_loader, tasks, device, report=True, logger=None, prefix=""):
        """
    ...
    def eval(self, model, return_preds_and_labels=False, calibrate_conf_scores=False):
        ...
        for head_num, head in enumerate(model.prediction_heads):
            ...
            result = {f"{self.prefix}-loss": loss_all[head_num] / len(self.data_loader.dataset), "task_name": head.task_name}
    ...
    def log_results(self, results, dataset_name, steps, logging=True, print=True, num_fold=None):
        # Print a header
        header = "\n\n"
        header += BUSH_SEP + "\n"
        header += "***************************************************\n"
            ...
            for metric_name, metric_val in head.items():
                ...
                if logging:
                    if not metric_name in ["preds", "labels"] and not metric_name.startswith("_"):
                        if isinstance(metric_val, numbers.Number) and self.logger:
                            self.logger.log_metrics(
                                metrics={
                                    f"{dataset_name.upper()}_{metric_name.lower()}_{head['task_name'].lower()}": metric_val
                                },
                                step=steps,
                            )
