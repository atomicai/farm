...
class TextClassificationHead(PredictionHead):
    def __init__(
        ...
        **kwargs,
        ...
        loss_fn = "crossentropy" if loss_fn is None else loss_fn
        if not loss_fn or loss_fn == "crossentropy":
            self.loss_fct = loss.CELoss(class_weights=balanced_weights, reduction=loss_reduction, ignore_index=-100)
        elif loss_fn == "focal":
            self.loss_fct = loss.FocalLoss(**kwargs)
...
class ArcTextClassificationHead(TextClassificationHead):
    def __init__(
        self,
        in_features,
        out_features,
        ...
        **kwargs,
        ...
        :param kwargs:
        """
        super(ArcTextClassificationHead, self).__init__()
        self.loss_fn = loss.AngularPenaltySMLoss(in_features, out_features, loss_type=loss_fn, **kwargs)
