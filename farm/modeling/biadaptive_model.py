...
class BiAdaptiveModel(nn.Module, BaseBiAdaptiveModel):
    ...
    def forward_lm(self, **kwargs):
        ...
        if "query_input_ids" in kwargs.keys():
            ...
            # seq_out_distill, pooled_out_distill = self.language_model1(
            #     input_ids=kwargs["query_input_ids"], padding_mask=kwargs["query_attention_mask"]
            # )
            pooled_output1 = self.language_model1(**kwargs)
    ...
    def log_params(self):
        """
        Logs paramteres to generic `BaseLogger`
        """
        params = {
