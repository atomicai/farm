...
def f1_micro(preds, labels):
    return {"f1_micro": f1_score(y_true=labels, y_pred=preds, average="micro")}
...
def compute_report_metrics(head, preds, labels):
    ...
    if head.ph_output_type in ["per_sequence"]:
        ...
        return report_fn(labels, preds, digits=4, labels=all_possible_labels, target_names=head.label_list, **extras)
    else:
