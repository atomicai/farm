...
from farm.utils import Benchmarker, WANDBLogger, calc_chunksize, initialize_device_settings, log_ascii_workers, set_all_seeds
...
class QAInferencer(Inferencer):
    ...
    ) -> Union[List[QAPred], Generator[QAPred, None, None]]:
        if isinstance(self.processor, NaturalQuestionsProcessor):
            for questions_key in ['questions', 'qas']:
                if questions_key in dicts[0].keys() and any([len(dict[questions_key]) > 1 for dict in dicts]):
                    logger.warning(
                        'More than one question for document. NaturalQuestions inference will return just the answer to the first question.'
                    )
